...
type peerController struct {
	...
	// htlcs maps incoming circuit key to timestamp that the htlc was
	// reported to us.
...
func (p *peerController) syncPendingHtlcs(ctx context.Context) (bool, error) {
	...
	for key := range p.htlcs {
		...
		if err := p.markHtlcComplete(ctx, key, nil); err != nil {
			return false, err
		}
...
func (p *peerController) run(ctx context.Context) error {
	...
	for {
		...
		case resolvedEvent := <-p.resolvedChan:
			...
			err := p.markHtlcComplete(ctx, key, &resolvedEvent)
			if err != nil {
				return err
			}
...
func (p *peerController) markHtlcComplete(ctx context.Context, key circuitKey,
	resolution *resolvedEvent) error {
	...
	// If no resolution is provided, we don't know the outcome of this
	// HTLC (we re-synced and it was no longer present), so there is no
	...
	return p.htlcCompleted(ctx, htlcInfo)
}
